...
function DashboardHero({ clients, passages, rdvs, saisonNow, isMobile, onAddPassage, onAddLivraison, onAddClient, onAddRdv }) {
...
function CarnetView({ client, passages, onRefresh, refreshing }) {
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          ...
          <div className="cv-glass" style={{borderRadius:12,padding:"11px 13px"}}>
            ...
            <div style={{marginTop:6,height:3,background:"rgba(255,255,255,0.2)",borderRadius:2,overflow:"hidden"}}>
              <div style={{"--pw":`${progressPct}%`,width:`${progressPct}%`,height:"100%",background:"rgba(255,255,255,0.8)",borderRadius:2,animation:"cv-progress 1s ease both 0.4s"}}/>
            </div>
          </div>
        </div>
      </div>
  ...
  const SectionHead = ({icon, title, action, onAction}) => (
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 0 8px"}}>
      ...
      {action && <button onClick={onAction} style={{fontSize:12,color:"#0891b2",border:"none",background:"none",cursor:"pointer",fontFamily:"inherit",fontWeight:500,padding:"4px 8px",borderRadius:6,background:"#f0f9ff"}}>{action} →</button>}
    </div>
  );
